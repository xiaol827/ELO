...
from buffer_offload import HostBufferManager
import os
import numpy as np
...
def compute_pes_grad(
  ...
  return jnp.mean(
      (pos_task_loss + neg_task_loss) / 2.0), es_grad, new_accumulator, p_ys, delta_losses, delta_task_losses, delta_direction_ori, delta_magnitude_ori
...
def compute_pes_grad_sharded_elo(
  ...
  (loss, es_grad, new_accumulator, p_ys_out, delta_losses,
   delta_task_losses, delta_direction_ori, delta_magnitude_ori) = _pes_grad_sharded_inner_elo(
...
class TruncatedPES_ELO(gradient_learner.GradientEstimator):
  ...
  def __init__(
      ...
      adaptive_buffer_thred: bool = False,
      bc_type: str = 'elo',
    ...
    self.adaptive_buffer_thred = adaptive_buffer_thred
    ...
    # Host buffer manager: buffer_cfg comes from truncated_step (VectorizedLOptTruncatedStep_ELO)
    buffer_cfg = getattr(truncated_step, 'buffer_cfg', {})
    # self.buffer_manager = HostBufferManager(buffer_cfg, truncated_step.num_tasks) if buffer_cfg else None
  ...
  def compute_gradient_estimate(  # pytype: disable=signature-mismatch  # overriding-parameter-type-checks
    ...
    # Update buffer threshold decay if adaptive
    if self.buffer_manager is not None and self.adaptive_buffer_thred:
      self.buffer_manager.update_thred(worker_weights.outer_state.outer_iteration)
    ...
    for i in range(self.trunc_length // self.steps_per_jit):
      ...
      # Buffer: prepare buffer candidates for all tasks before scan window
      if self.buffer_manager is not None:
        p_state, n_state = self.buffer_manager.pre_window(
            p_state, n_state, next(rng))
        prev_p_steps = np.array(jax.device_get(p_state.inner_step))
      ...
      # Buffer push: save states that crossed idx2push
      if self.buffer_manager is not None:
        self.buffer_manager.post_window(p_state, n_state, prev_p_steps)
    ...
    (loss, es_grad, new_accumulator, p_ys, delta_losses, delta_task_losses,
     delta_direction_ori, delta_magnitude_ori) = self.grad_fn(
        dirloss_weight,
        magloss_weight,
        p_yses,
        n_yses,
...
class TruncatedPESPMAP_ELO(TruncatedPES_ELO):
  ...
  def compute_gradient_estimate(
    ...
    (loss, es_grad, new_accumulator, p_ys, delta_losses, delta_task_losses,
     delta_direction_ori, delta_magnitude_ori) = self.pmap_compute_pes_grad(
