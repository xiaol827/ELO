...
class TruncatedUnrollState:
  ...
  # Buffer candidates for in-scan reset (populated by HostBufferManager before scan)
  pending_buffer_opt_state: Any = None
  pending_buffer_task_param: Any = None
  pending_buffer_inner_step: Any = None
  use_buffer_on_reset: Any = None
...
def _init_truncation_state(
  ...
  return TruncatedUnrollState(  # pytype: disable=wrong-arg-types  # jax-ndarray
      ...
      is_done=False,
      pending_buffer_opt_state=jax.tree_util.tree_map(jnp.zeros_like, opt_state),
      pending_buffer_task_param=jax.tree_util.tree_map(jnp.zeros_like, task_param),
      pending_buffer_inner_step=jnp.asarray(0, dtype=jnp.int32),
      use_buffer_on_reset=jnp.asarray(False),
  )
...
def progress_or_reset_inner_opt_state(
    ...
    is_done: bool,
    data: Any,
    pending_buffer_opt_state=None,
    pending_buffer_task_param=None,
    pending_buffer_inner_step=0,
    use_buffer_on_reset=False,
    cond_fn: Callable[[bool, Any, Any, Any], Any] = jax.lax.cond,
    axis_name: Optional[str] = None,
    meta_loss_with_aux_key: Optional[str] = None,
) -> Tuple[T, G, int, jnp.ndarray, jnp.ndarray]:
  """Train a single step, or reset the current inner problem.

  When is_done:
    - use_buffer_on_reset=True  → restore from pending buffer state
    - use_buffer_on_reset=False → random init (sample new task)
  """
  summary.summary("num_steps", num_steps, aggregation="sample")  # pytype: disable=wrong-arg-types  # jax-ndarray

  def random_init_fn(key):
    """Reset the state of the inner-problem with random init."""
    if axis_name:
    ...
    return next_inner_opt_state, task_param, jnp.asarray(0, dtype=jnp.int32), jnp.asarray(0.), jnp.asarray(0.), jnp.asarray(0.), jnp.asarray(0.)

  def buffer_init_fn(key):
    """Restore from pre-loaded buffer state."""
    return (pending_buffer_opt_state, pending_buffer_task_param,
            jnp.asarray(pending_buffer_inner_step, dtype=jnp.int32),
            jnp.asarray(0., dtype=jnp.float32), jnp.asarray(0., dtype=jnp.float32),
            jnp.asarray(0., dtype=jnp.float32), jnp.asarray(0., dtype=jnp.float32))

  def reset_fn(key):
    """Decide between buffer init and random init."""
    return jax.lax.cond(use_buffer_on_reset, buffer_init_fn, random_init_fn, key)

  def train_fn(key, inner_opt_state=inner_opt_state):
    ...
    imt_loss = cosine_loss + magnitude_loss
    next_inner_step = inner_step + 1

    return next_inner_opt_state, task_param, next_inner_step, jnp.asarray(
        meta_loss, dtype=jnp.float32), jnp.asarray(imt_loss, dtype=jnp.float32), jnp.asarray(
        cosine_loss, dtype=jnp.float32), jnp.asarray(magnitude_loss, dtype=jnp.float32)


  next_inner_opt_state, task_param, next_inner_step, meta_loss, imt_loss, cosine_loss, magnitude_loss = cond_fn(
      jnp.logical_not(is_done), train_fn, reset_fn, key)

  return next_inner_opt_state, task_param, next_inner_step, meta_loss, imt_loss, cosine_loss, magnitude_loss
...
def _truncated_unroll_one_step(
  ...
  When is_done=True, decides buffer_init vs random_init based on
  state.use_buffer_on_reset (set by HostBufferManager before scan window).
  """
  key1, key2, key3 = jax.random.split(key, 3)
  ...
  # Track whether this step will use buffer init
  used_buffer = jnp.logical_and(state.is_done, state.use_buffer_on_reset)

  next_inner_opt_state, task_param, next_inner_step, task_loss, imt_loss, cosine_loss, magnitude_loss = (
      progress_or_reset_inner_opt_state(  # pytype: disable=wrong-arg-types  # jax-ndarray
          ...
          data=data,
          pending_buffer_opt_state=state.pending_buffer_opt_state,
          pending_buffer_task_param=state.pending_buffer_task_param,
          pending_buffer_inner_step=state.pending_buffer_inner_step,
          use_buffer_on_reset=state.use_buffer_on_reset,
          meta_loss_with_aux_key=meta_loss_with_aux_key,
      )
  )

  # Normal truncation state update
  next_truncation_state, is_done = trunc_sched.next_state(state.truncation_state, next_inner_step, key2, outer_state)

  # Buffer init override: extend truncation length so task doesn't immediately re-trigger is_done
  buffer_trunc = trunc_sched.init(key3, outer_state)
  adjusted_length = state.pending_buffer_inner_step + buffer_trunc.length

  # Merge: use buffer trunc state if buffer was used, otherwise keep normal
  final_trunc = jax.tree_util.tree_map(
      lambda b, n: jnp.where(used_buffer, b, n),
      buffer_trunc, next_truncation_state)
  final_trunc = final_trunc.replace(
      length=jnp.where(used_buffer, adjusted_length, next_truncation_state.length))
  final_is_done = jnp.where(used_buffer, False, is_done)

  # Clear use_buffer_on_reset after any reset (so 2nd reset in same window uses random)
  new_use_buffer = jnp.where(state.is_done, False, state.use_buffer_on_reset)
  ...
  output_state = TruncatedUnrollState(  # pytype: disable=wrong-arg-types  # jax-ndarray
      ...
      truncation_state=final_trunc,
      task_param=task_param,
      is_done=final_is_done,
      pending_buffer_opt_state=state.pending_buffer_opt_state,
      pending_buffer_task_param=state.pending_buffer_task_param,
      pending_buffer_inner_step=state.pending_buffer_inner_step,
      use_buffer_on_reset=new_use_buffer,
  )

  # Mask out reset steps: both random_init (step==0) and buffer_init (used_buffer)
  step_mask = jnp.logical_and(next_inner_step != 0, jnp.logical_not(used_buffer))
