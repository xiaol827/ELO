...
class TruncatedUnrollState:
  ...
  push_at_step: jnp.ndarray   # int32, the inner_step at which we push this trajectory
...
def _init_truncation_state(
  ...
  push_at_step = jax.random.randint(
      key5, shape=(), minval=0, maxval=jnp.maximum(num_steps, 1))
  return TruncatedUnrollState(  # pytype: disable=wrong-arg-types  # jax-ndarray
      ...
      push_at_step=jnp.asarray(push_at_step, dtype=jnp.int32),
...
def progress_or_reset_inner_opt_state(
...
) -> Tuple[T, G, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, Any, Any, jnp.ndarray, Any, jnp.ndarray]:
  ...
  fresh opt_state. During training, when `inner_step == push_at_step` we write
  the just-updated state into `next_write_slot` and advance the slot cursor.
  """
  ...
  def reset_fn(key):
    ...
    (key_choice, key_rand, key_push) = jax.random.split(key, 3)
    ...
    def buffer_branch(rng):
      ...
      return restored_state, restored_task, restored_step

    next_inner_opt_state, new_task_param, next_inner_step = jax.lax.cond(
        use_random, random_branch, buffer_branch,
    ...
    # Regenerate push_at_step uniformly within [next_inner_step, effective_length).
    hi = jnp.maximum(effective_length, next_inner_step + 1)
    new_push_at_step = jax.random.randint(
        key_push, shape=(), minval=next_inner_step, maxval=hi)
    # new_push_at_step = next_inner_step + (effective_length - next_inner_step) // 2
    ...
    return (next_inner_opt_state, new_task_param, next_inner_step,
            ...
            jnp.asarray(new_push_at_step, dtype=jnp.int32))
  ...
  def train_fn(key):
    ...
    # Push current state into buffer when we hit push_at_step.
    should_push = inner_step == push_at_step
    new_state_buffer = jax.lax.cond(
        should_push,
        lambda: _write_buffer_slot(state_buffer, next_inner_opt_state, next_write_slot),
        lambda: state_buffer)
    new_task_param_buffer = jax.lax.cond(
        should_push,
        lambda: _write_buffer_slot(task_param_buffer, task_param, next_write_slot),
        lambda: task_param_buffer)
    new_next_write_slot = jnp.where(
        should_push,
        (next_write_slot + 1) % buffer_size,
        next_write_slot,
    )

    # Advance truncation schedule using the unchanged length from the
    # incoming truncation_state.
    ...
    return (next_inner_opt_state, task_param, next_inner_step,
            ...
            new_state_buffer, new_task_param_buffer, new_next_write_slot,
            truncation_state, push_at_step)
...
def _truncated_unroll_one_step(
  ...
  (next_inner_opt_state, task_param, next_inner_step, task_loss,
   ...
   new_next_write_slot, next_truncation_state, new_push_at_step) = (
       progress_or_reset_inner_opt_state(  # pytype: disable=wrong-arg-types  # jax-ndarray
           ...
           push_at_step=state.push_at_step,
  ...
  output_state = TruncatedUnrollState(  # pytype: disable=wrong-arg-types  # jax-ndarray
      ...
      push_at_step=jnp.asarray(new_push_at_step, dtype=jnp.int32),
