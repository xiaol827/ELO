...
class MuDepthMLP(hk.Module):
  ...
  def __init__(
    ...
    with_bias=False # no biases with mudepth

    self.depth = len(output_sizes)
    self.depth_mult = depth_mult / jnp.sqrt(self.depth)
    self.depth_lr_mult = depth_lr_mult / jnp.sqrt(self.depth)
    ...
    for index, output_size in enumerate(output_sizes):
      if index ==0:
        ...
        self.get_adam_mup_lr_mul["~/linear_%d"  % index] = {'w':1.0}
        ...
        layers.append(hk.Linear(output_size=output_size,
                                ...
                                with_bias=with_bias,
                                name="linear_%d" % index))
        self.get_adam_mup_lr_mul["~/linear_%d"  % index] = {'w': hidden_lr_mult / output_sizes[index-1]}
  ...
  def __call__(
    ...
    for i, layer in enumerate(self.layers):
      ...
      if i == 0:
        ...
        out = layer(out) * self.hidden_mult * self.depth_mult
...
class _MuDepthMLPImageTask(_MLPImageTask, MuTask):
