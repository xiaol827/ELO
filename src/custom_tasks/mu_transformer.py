...
class MuCausalSelfAttention(hk.Module):
  ...
  def __init__(
    ...
    adam_lr_mul_kqv = {'w': depth_mult_lr * ( hidden_lr_mult / (self.model_size)), 'b': depth_mult_lr}
    adam_lr_mul_out = {'w': depth_mult_lr * ( hidden_lr_mult / (self.value_size * self.num_heads)), 'b': depth_mult_lr}
...
class MuDenseBlock(hk.Module):
  ...
  def __init__(self,
    ...
    hk.set_state("mup_lrs",{'linear':  {'w': ( hidden_lr_mult / d_model) * depth_mult_lr, 
                                        'b': depth_mult_lr},

                            'linear_1':  {'w': ( hidden_lr_mult / (widening_factor * d_model)) * depth_mult_lr, 
                                          'b': depth_mult_lr},})
...
class MuTransformer(hk.Module):
  ...
  def __init__(self,
               ...
               hidden_lr_mult: float = 1.0,
               use_mu_depth: bool = False,
               use_complete_p: bool = False):
    super().__init__(name=name)
    ...
    self._use_mu_depth = use_mu_depth
    self._use_complete_p = use_complete_p

    if self._use_mu_depth:
      self._depth_mult_lr = 1 / ( num_layers * 2 )
      self._depth_mult_residual = 1 / ( num_layers * 2 )
    elif self._use_complete_p:
      self._depth_mult_lr = 1.0
      self._depth_mult_residual = 1.0 / ( num_layers * 2 )
    else:
      self._depth_mult_lr = 1.0
      self._depth_mult_residual = 1.0
    ...
    adam_lr_mul_in = {'embeddings': 1.0}
    adam_lr_mul_out = {'w': self._depth_mult_lr, 'b': self._depth_mult_lr}
    adam_lr_mul_ln = {'scale': self._depth_mult_lr, 'offset': self._depth_mult_lr}
    ...
    hk.set_state("eps_mult", 1 / self._d_model)
  ...
  def __call__(self, h: jnp.ndarray, mask: Optional[jnp.ndarray],
    ...
    for i in range(self._num_layers):
      ...
      h_attn = MuCausalSelfAttention(
          ...
          name=f"h{i}_attn",
          depth_mult_lr=self._depth_mult_lr,
          hidden_lr_mult=self._hidden_lr_mult)(
            h_norm, mask=mask)
      
      h_attn = hk.dropout(hk.next_rng_key(), dropout_rate, h_attn)

      h = h + self._depth_mult_residual * h_attn 
      ...
      h = h + self._depth_mult_residual * h_dense
...
class _MuTransformerTask(base.Task, MuTask):
  ...
  def _hk_forward(self, batch):
    ...
    mod = MuTransformer(
        ...
        use_mu_depth=self._use_mu_depth,
        use_complete_p=self._use_complete_p,
        **self.mup_multipliers)
    mask = (batch['image'] != self.eos_token_id)
  ...
  def init_with_state(self, key: chex.PRNGKey) -> base.Params:
    ...
    return params, self.get_mup_state(state, eps_mult=state['mu_transformer']['eps_mult'])
...
def test_causal_mask():
    """Test the causal mask creation in MuCausalSelfAttention."""
    print("=" * 60)
    print("Testing Causal Mask Creation")
    print("=" * 60)
    
    # Test 1: Basic causal mask creation
    print("\nTest 1: Basic causal mask (seq_len=5)")
    seq_len = 5
    causal_mask = onp.tril(onp.ones((seq_len, seq_len)))
    print("Causal mask shape:", causal_mask.shape)
    print("Causal mask:\n", causal_mask.astype(int))
    print("Expected: Lower triangular matrix with 1s")
    
    # Test 2: Larger sequence length
    print("\nTest 2: Larger causal mask (seq_len=8)")
    seq_len = 8
    causal_mask = onp.tril(onp.ones((seq_len, seq_len)))
    print("Causal mask shape:", causal_mask.shape)
    print("Causal mask:\n", causal_mask.astype(int))
    
    # Test 3: Mask combination with padding mask
    print("\nTest 3: Combining causal mask with padding mask")
    seq_len = 5
    causal_mask = onp.tril(onp.ones((seq_len, seq_len)))
    # Simulate a padding mask where last 2 tokens are padding
    padding_mask = onp.array([[1, 1, 1, 0, 0]])  # Shape: (1, 5)
    # Expand to match attention shape: (1, 1, 1, 5) for broadcasting
    padding_mask_expanded = padding_mask[None, None, :, :]
    print("Padding mask (last 2 positions are padding):", padding_mask)
    print("Causal mask:\n", causal_mask.astype(int))
    
    # Combine masks
    combined_mask = padding_mask * causal_mask
    print("Combined mask (causal AND padding):\n", combined_mask.astype(int))
    print("Note: Positions 3,4 should be zero in all rows")
    
    # Test 4: Verify mask dimensions for multi-head attention
    print("\nTest 4: Mask broadcasting for multi-head attention")
    batch_size = 2
    num_heads = 4
    seq_len = 5
    ...
    print(f"Attention logits shape would be: ({batch_size}, {num_heads}, {seq_len}, {seq_len})")
    print(f"Causal mask will broadcast from {causal_mask.shape} to attention shape")
    
    # Test 5: Test the actual masking behavior in attention
    print("\nTest 5: Effect on attention logits")
    seq_len = 4
    causal_mask = onp.tril(onp.ones((seq_len, seq_len)))
    # Simulate some attention logits
    fake_logits = onp.random.randn(1, 1, seq_len, seq_len)
    print("Random attention logits:\n", fake_logits[0, 0])
    print("\nCausal mask:\n", causal_mask.astype(int))
    # Apply masking as done in the code
    masked_logits = onp.where(causal_mask, fake_logits, -1e30)
    print("\nMasked logits (unmasked positions get -1e30):\n", masked_logits[0, 0])
    print("\nNote: Upper triangle should have very negative values (-1e30)")
    
    # Test 6: Verify softmax behavior after masking
    print("\nTest 6: Softmax behavior after masking")
    from scipy.special import softmax as np_softmax
    attention_weights = np_softmax(masked_logits[0, 0], axis=-1)
    print("Attention weights after softmax:\n", attention_weights)
    print("Row sums (should be 1.0):", attention_weights.sum(axis=-1))
    print("Note: Upper triangle should be approximately 0 after softmax")
