...
class MuCausalSelfAttention(hk.Module):
  ...
  def __init__(
    ...
    # Create mup_lrs dictionary with separate entries for each head
    mup_lrs = {'linear': adam_lr_mul_out}
    
    # Add separate learning rates for each query head
    for i in range(self.num_heads):
      mup_lrs[f'query_head_{i}'] = adam_lr_mul_kqv
    
    # Add separate learning rates for each key head
    for i in range(self.num_heads):
      mup_lrs[f'key_head_{i}'] = adam_lr_mul_kqv
    
    # Add separate learning rates for each value head
    for i in range(self.num_heads):
      mup_lrs[f'value_head_{i}'] = adam_lr_mul_kqv
  ...
  def __call__(
    ...
    for i in range(self.num_heads):
      # Project query, key, value for each head separately
      q_head = hk.Linear(self.key_size, 
                         w_init=self._hidden_w_init,
                         b_init=self._b_init, 
                         name=f"query_head_{i}")(query)
      
      k_head = hk.Linear(self.key_size, 
                         w_init=self._hidden_w_init, 
                         b_init=self._b_init, 
                         name=f"key_head_{i}")(key)
      
      v_head = hk.Linear(self.value_size, 
                         w_init=self._hidden_w_init,
                         b_init=self._b_init, 
                         name=f"value_head_{i}")(value)
      
      # Add head dimension
      query_heads.append(q_head[:, :, None, :])
      key_heads.append(k_head[:, :, None, :])
      value_heads.append(v_head[:, :, None, :])
    
    # Concatenate along the head dimension
    query_heads = jnp.concatenate(query_heads, axis=2)  # [B, T, num_heads, head_size]
    key_heads = jnp.concatenate(key_heads, axis=2)      # [B, T, num_heads, head_size]
    value_heads = jnp.concatenate(value_heads, axis=2)  # [B, T, num_heads, head_size]

    # Transpose to get dimensions [B, T, num_heads, head_size] -> [B, num_heads, T, head_size]
    ...
    # Compute attention
    attn_logits = jnp.einsum("...htd,...hTd->...htT", query_heads, key_heads)
    sqrt_key_size = self.key_size
    ...
    attn = jnp.einsum("...htT,...hTd->...htd", attn_weights, value_heads)
    
    # Transpose back and reshape
    attn = jnp.transpose(attn, (0, 2, 1, 3))  # [B, T, num_heads, head_size]
