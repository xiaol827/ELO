...
def second_moment_normalizer(x, axis, key, eps=1e-5, num_params=None,):
  if num_params is None:
    # Original behavior: use all parameters
    return x * lax.rsqrt(eps + jnp.mean(jnp.square(x), axis=axis, keepdims=True))
  else:
    # Use only the first num_params parameters for normalization
    # Take a slice of x along the specified axis
    # Handle both single axis and list of axes
    if isinstance(axis, list):
        # Create a slice for each dimension
        slice_indices = [slice(None)] * x.ndim
        # For each axis in the list, create a slice of the first min(num_params, dim_size) elements
        if len(axis) == 2:
            num_params = int(num_params**0.5)
        elif len(axis) == 3:
            num_params = int(num_params**(1/3))

        # Use lax.dynamic_slice instead of slice with dynamic indices
        # First, create a full-sized array of zeros
        x_slice = jnp.zeros_like(x)
        
        # Create start_indices for dynamic_slice
        start_indices = [0] * x.ndim
        slice_sizes = list(x.shape)
        
        # Adjust start_indices and slice_sizes for the axes we want to sample
        for ax in axis:
            dim_size = x.shape[ax]
            slice_size = min(num_params, dim_size)
            offset = jax.random.randint(key, shape=(), minval=0, maxval=dim_size-slice_size)
            start_indices[ax] = offset
            slice_sizes[ax] = slice_size
        # print("x",x.shape)
        # print("start_indices",start_indices)
        # print("slice_sizes",slice_sizes)
        # Use dynamic_slice to extract the subset
        x_slice = lax.dynamic_slice(x, start_indices, slice_sizes)
        ...
        slice_size = min(num_params, dim_size)
        slice_indices[axis] = slice(0, slice_size)
        x_slice = x[tuple(slice_indices)]
    
    # Compute the normalization statistic using only the slice
    norm_stat = jnp.mean(jnp.square(x_slice), axis=axis, keepdims=True)
    
    # Apply the normalization to the entire tensor
    return x * lax.rsqrt(eps + norm_stat)
...
class MuAdafacMLPLOptv2(lopt_base.LearnedOptimizer):
  ...
  def __init__(self,
    ...
    self.dyn_tanh = dyn_tanh
    self.num_params_normalizer = num_params_normalizer
  ...
  def _mod(self, global_feat, p, g, m, rms, fac_g, fac_vec_col, fac_vec_row,
    ...
    if self.dyn_tanh:
      ...
      alpha = hk.get_parameter(
            "dyt_alpha", shape=[1], dtype=jnp.float32, 
            init=lambda *_: jnp.ones([1]) * 0.5)
      dyt_weight = hk.get_parameter(
          "dyt_weight", shape=[last_size], dtype=jnp.float32,
          init=jnp.ones)
      dyt_bias = hk.get_parameter(
          "dyt_bias", shape=[last_size], dtype=jnp.float32,
          init=jnp.zeros)
    ...
    # dyn_tanh = {}
    # for x in range(self._hidden_layers):
    #   dyn_tanh[f"alpha_{x}"] = hk.get_parameter(
    #         f"dyt_alpha_{x}", shape=[1], dtype=jnp.float32, 
    #         init=lambda *_: jnp.ones([1]) * 0.5)
    #   dyn_tanh[f"weight_{x}"] = hk.get_parameter(
    #       f"dyt_weight_{x}", shape=[self._hidden_size], dtype=jnp.float32,
    #       init=jnp.ones)
    #   dyn_tanh[f"bias_{x}"] = hk.get_parameter(
    #       f"dyt_bias_{x}", shape=[self._hidden_size], dtype=jnp.float32,
    #       init=jnp.zeros)
    ...
    if self._concat_weights:
      # concat the inputs, normalize using DyT-like normalization
      inp_stack = jnp.concatenate(inps, axis=-1)
      # Apply tanh normalization with learnable parameters
      axis = list(range(len(p.shape)))

      if self.dyn_tanh:
        inp_stack = jnp.tanh(alpha * inp_stack)
        inp_stack = inp_stack * dyt_weight + dyt_bias
      else:
        self.key, key = jax.random.split(self.key)
        inp_stack = second_moment_normalizer(inp_stack, axis=axis, key=key, num_params=self.num_params_normalizer)
      ...
      for wi, (w, b) in enumerate(zip(weights, biases)):
        ...
        if wi != len(weights) - 1:
          # net = jnp.tanh(dyn_tanh[f"alpha_{wi}"] * net)
          # net = net * dyn_tanh[f"weight_{wi}"] + dyn_tanh[f"bias_{wi}"]
      ...
      for wi, (w, b) in enumerate(zip(weights, biases)):
        ...
        # activation
        if wi != len(weights) - 1:
          inp = [self.non_linearity(x) for x in inp]
  ...
  def opt_fn(self,
    ...
    class _Opt(opt_base.Optimizer):
      ...
      def _get_rolling(self):
        ...
        # param_to_decay(
        #     decay_to_param() #+  # pylint: disable=protected-access
