...
@gin.configurable
class MuAdafacMLPLOptV4BC(lopt_base.LearnedOptimizer):
  ...
  def __init__(self,
    ...
    self.non_linearity = jax.nn.relu #lambda x: x * jax.nn.sigmoid(x)  # swish activation
    ...
    if teacher_force_schedule:
      self.teacher_force_schedule = get_optax_schedule(teacher_force_schedule['class_'], teacher_force_schedule['kwargs'])
    else:
      self.teacher_force_schedule = None
  ...
  def _mod(self, 
           ...
           mup_lr_scale,
           global_epsilon,):  # Learning rate scale from muP (width aware scaling)
    """LO forward pass"""
    ...
    if not p.shape:
      ...
      global_epsilon = jnp.expand_dims(global_epsilon, 0)
      did_reshape = True
    else:
      did_reshape = False
    inps = []
    ...
    if factored_dims is not None:
      ...
      row_factor = common.safe_rsqrt(fac_vec_row / (row_col_mean + global_epsilon), epsilon=global_epsilon)
      col_factor = common.safe_rsqrt(fac_vec_col, epsilon=global_epsilon)
      fac_mom_mult = (
      ...
      fac_mom_mult = m * (fac_vec_v + global_epsilon)**-0.5
      inps.append(fac_mom_mult)
    ...
    inp_stack = second_moment_normalizer(inp_stack, axis=axis, eps=global_epsilon)
  ...
  def init(self, key: PRNGKey) -> lopt_base.MetaParams:
    ...
    mod_theta = self._mod_init(key, global_features, p, g, m, rms, fac_g,
                               fac_vec_col, fac_vec_row, fac_vec_v, mup_lr_scale, epsilon_tree)
    ...
    class _Opt(opt_base.Optimizer):
      ...
      def update(self,
        ...
        next_fac_rolling_features, fac_g = fac_vec_roll.update(
            opt_state.fac_rolling_features, grad, local_epsilon=global_epsilon)
        ...
        if parent.train:
          ...
          output_params = next_params

          if parent.teacher_force_schedule:
            output_params = jax.tree_util.tree_map(
                lambda x, y: (1 - parent.teacher_force_schedule(parent.outer_step)) * x + parent.teacher_force_schedule(parent.outer_step) * y,
                next_params,
                bc_opt_state.params)
