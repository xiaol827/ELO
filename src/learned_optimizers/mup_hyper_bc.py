...
def _safe_rsqrt(x, epsilon=1e-9):
  return lax.rsqrt(jnp.maximum(x, epsilon))
...
@gin.configurable
class MuHyperV2BC(lopt_base.LearnedOptimizer):
  ...
  def __init__(
    ...
    self.bc_optimizer_args = bc_optimizer_args[jax.process_index()]
  ...
  def _rnn_forward(self, x, state):
    ...
    if self.mix_layers:
      ...
      #FIXED: 
      mix_layer = hk.Linear(self.lstm_hidden_size)(mix_layer)
      mix_layer = jax.nn.relu(mix_layer)
  ...
  def _ff_mod(self, global_feat, extra_step_mult, p, g, m, rms, fac_g,
              fac_vec_col, fac_vec_row, fac_vec_v, mup_lr_scale, global_epsilon):
    ...
    if self.with_rms_norm_g or self.with_rsqrt_rms and self.with_rms_only_norm_g:
      rsqrt = lax.rsqrt(rms + global_epsilon)
    ...
    if self.adafactor_accumulator:
      ...
      if factored_dim is not None:
        ...
        if self.with_fac_rsqrt:
          inps.append(lax.rsqrt(row_feat + global_epsilon))
          inps.append(lax.rsqrt(col_feat + global_epsilon))
        ...
        row_factor = _safe_rsqrt(fac_vec_row / (row_col_mean + global_epsilon), epsilon=global_epsilon)
        col_factor = _safe_rsqrt(fac_vec_col, epsilon=global_epsilon)
        ...
        if self.with_fac_rsqrt:
          inps.append(lax.rsqrt(fac_vec_v + global_epsilon))
          inps.append(lax.rsqrt(fac_vec_v + global_epsilon))
    ...
    inp_stack = [_second_moment_normalizer(i, axis=axis, eps=global_epsilon) for i in inps]
    ...
    param_scale = jnp.sqrt(jnp.mean(jnp.square(p)) + global_epsilon)
    ...
    if self.precondition_output:
      ...
      step = step * lax.rsqrt(norms + global_epsilon)
  ...
  def init(self, key) -> lopt_base.MetaParams:
    ...
    epsilon_tree = jnp.array([self.epsilon])
    ...
    def ffmod_init(key):
      ...
      mod_theta = self.ff_mod.init(key, global_features, 1.0, p, g, m, rms,
                                   fac_g, fac_vec_col, fac_vec_row, fac_vec_v, mup_lr_scale, epsilon_tree)
      return mod_theta
  ...
  def opt_fn(self, theta, is_training=True) -> opt_base.Optimizer:
    ...
    class _Opt(opt_base.Optimizer):
      ...
      def update(self,
        ...
        global_epsilon = model_state['eps_mult']['eps_mult'] * parent.epsilon
        # print(global_epsilon)
        epsilon_tree = jax.tree_util.tree_map(lambda x: global_epsilon, lrs)
        # print(epsilon_tree)
        ...
        next_adafac_rolling, fac_g = adafac_roll.update(opt_state.fac_rolling,
                                                        grads, local_epsilon=global_epsilon)
        ...
        if parent.train:
          ...
          def compute_bc_gradients(net_params, grads, lstm_hidden_state, key):
            ...
            def loss_fn(params_opt, key):
              ...
              def apply_one(control_param, key, lr_mult, p, g, m, rms, fac_g, v_col,
                      v_row, v, mup_lr_scale, global_epsilon):
              ...
              next_params = jax.tree_util.tree_map(
                  ...
                  next_adafac_rolling.v_diag, mup_lr_scale, epsilon_tree)
              ...
              return loss, (loss, next_params, next_lstm_hidden_state)
            
            # Compute gradient and auxiliary values with respect to network parameters
            grad_fn = jax.value_and_grad(loss_fn, has_aux=True)
            (loss, (aux_loss, next_params, next_lstm_hidden_state)), grads = grad_fn(net_params, key)
            
            return grads, loss, next_params, next_lstm_hidden_state

          grads, loss, next_params, next_lstm_hidden_state = compute_bc_gradients(self.theta, grads, lstm_hidden_state, key)
          grads['bc_loss'] = loss
          ...
          return tree_utils.match_type(ss, opt_state), grads
        else:
