        ...
        if parent.train:
          ...
          def compute_bc_gradients(net_params):
            ...
            def loss_fn(params):
              ...
              # Helper function to apply one step of Adam update to each tensor
              def adam_update(param, g, m, v, lr, alr):
                # Bias correction is handled in the rolling averages
                epsilon = 1e-8
                update = m / (jnp.sqrt(v) + epsilon)
                return param - alr * lr * update
              
              # Apply Adam update to each parameter
              next_params_adam = jax.tree_util.tree_map(
                  lambda p, g, m, v, lr, alr: adam_update(p, g, m, v, lr, alr),
                  opt_state.params,
                  grad,
                  jax.tree_util.tree_map(lambda x: x[..., 0], next_mom_rolling.m),
                  jax.tree_util.tree_map(lambda x: x[..., 0], next_rms_rolling.rms),
                  lrs,
                  jax.tree_util.tree_map(lambda x: parent.adam_lr, lrs))
              ...
              loss = mse_loss( jax.tree_util.tree_map(lambda x,y: x - y, next_params, opt_state.params), 
                              jax.tree_util.tree_map(lambda x,y: x - y, next_params_adam, opt_state.params))
              
              return loss, (loss, next_params, next_params_adam)
            
            # Compute gradient and auxiliary values with respect to network parameters
            grad_fn = jax.value_and_grad(loss_fn, has_aux=True)
            (loss, (aux_loss, next_params, next_params_adam)), grads = grad_fn( net_params)
            
            return grads, loss, next_params_adam, next_params
          ...
          grads, loss, next_params_adam, next_params = compute_bc_gradients(self.theta)
          # print(jax.tree_util.tree_map(lambda x: x.shape, grads))
          # exit(0)

          grads['bc_loss'] = loss

          next_opt_state = AdafacMLPLOptState(
              params=next_params_adam if parent.teacher_force else next_params,
