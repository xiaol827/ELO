...
@gin.configurable
class MuAdafacMLPLOptv2(lopt_base.LearnedOptimizer):
  ...
  def _mod(self, global_feat, p, g, m, rms, fac_g, fac_vec_col, fac_vec_row,
    ...
    #     if wi == 0:
    #       weights[wi] = jnp.ones_like(weights[wi])
    #       biases[wi] = jnp.zeros_like(biases[wi])

    #   last_size = w


    # Initialize weights for skip connection architecture with SwiGLU blocks
    stddev = 1. / onp.sqrt(last_size)
    w_init = hk.initializers.TruncatedNormal(stddev=stddev)
    
    # Output dimension is always 2 (direction and magnitude)
    output_dim = 2
    
    # Create initial projection and residual weights
    if self._make_separate_weights or self._concat_weights:
      # Initial projection from inputs to hidden dimension
      weights.append(
          hk.get_parameter(
              "w_initial", shape=(last_size, self._hidden_size), dtype=jnp.float32, init=w_init))
      biases.append(
          hk.get_parameter(
              "b_initial", shape=(self._hidden_size,), dtype=jnp.float32, init=jnp.zeros))
      
      # Direct residual connection from input to output
      weights.append(
          hk.get_parameter(
              "w_residual", shape=(last_size, output_dim), dtype=jnp.float32, init=w_init))
      biases.append(
          hk.get_parameter(
              "b_residual", shape=(output_dim,), dtype=jnp.float32, init=jnp.zeros))
      
      for i in range(self._hidden_layers):
        # Up projection for SwiGLU (2x for gate and value)
        expanded_dim = 2 * self._hidden_size * self.expansion_factor
        # Use scaled initialization for better training dynamics
        fan_in = self._hidden_size
        fan_out = expanded_dim
        limit = jnp.sqrt(6.0 / (fan_in + fan_out))
        w_init_up = hk.initializers.UniformScaling(scale=limit)
        weights.append(
            hk.get_parameter(
                f"w_up_{i}", shape=(self._hidden_size, expanded_dim), dtype=jnp.float32, init=w_init_up))
        # Initialize biases to small values for gate part and zeros for value part
        # Create bias initialization with small positive values for gate part
        gate_size = self._hidden_size * self.expansion_factor
        b_init_values = jnp.concatenate([
            jnp.full((gate_size,), 0.0, dtype=jnp.float32),  # Gate part with small positive bias
            jnp.zeros((expanded_dim - gate_size,), dtype=jnp.float32)  # Value part with zeros
        ])
        biases.append(
            hk.get_parameter(
                f"b_up_{i}", shape=(expanded_dim,), dtype=jnp.float32, init=lambda *_: b_init_values))
        
        # Down projection - use smaller initialization to stabilize residual connections
        target_dim = self._hidden_size if i < self._hidden_layers - 1 else output_dim
        fan_in = self._hidden_size * self.expansion_factor
        fan_out = target_dim
        # Use scaled initialization with a smaller factor for projection layers
        limit = jnp.sqrt(6.0 / (fan_in + fan_out)) * 0.1  # Scale down for stability
        w_init_down = hk.initializers.UniformScaling(scale=limit)
        weights.append(
            hk.get_parameter(
                f"w_down_{i}", shape=(self._hidden_size * self.expansion_factor, target_dim), dtype=jnp.float32, init=w_init_down))
        biases.append(
            hk.get_parameter(
                f"b_down_{i}", shape=(target_dim,), dtype=jnp.float32, init=jnp.zeros))
    
    # Forward pass function to be used later
    def forward_pass(inp_stack):
      # Initial projection
      hidden = inp_stack @ weights[0] + jnp.broadcast_to(biases[0], list(inp_stack.shape[0:-1]) + [self._hidden_size])
      hidden = self.non_linearity(hidden)
      
      # Direct residual connection from input to output
      residual = inp_stack @ weights[1] + jnp.broadcast_to(biases[1], list(inp_stack.shape[0:-1]) + [biases[1].shape[0]])
      ...
      for i in range(self._hidden_layers):
        ...
        # Up projection
        up_proj = hidden @ weights[w_up_idx] + jnp.broadcast_to(biases[w_up_idx], list(hidden.shape[0:-1]) + [biases[w_up_idx].shape[0]])
        
        # Split into gate and value parts
        gate_size = self._hidden_size * self.expansion_factor
        gate = self.non_linearity(up_proj[..., :gate_size])
        value = up_proj[..., gate_size:]
        
        # Apply SwiGLU activation
        activated = gate * value
        
        # Down projection
        down_proj = activated @ weights[w_down_idx] + jnp.broadcast_to(biases[w_down_idx], list(activated.shape[0:-1]) + [biases[w_down_idx].shape[0]])
        
        # Update hidden state (or set as output for final layer)
        if i < self._hidden_layers - 1:
            hidden = down_proj
        else:
            # Only add the residual connection at the final output
            output = down_proj + residual
  ...
  def opt_fn(self,
    ...
    class _Opt(opt_base.Optimizer):
      ...
      def update(self,
        ...
        # print("\n\n\n")
        # import pprint
        # pprint.pprint(jax.tree_util.tree_map(lambda x:type(x), grad))
        # print("\n\n\n")
        # pprint.pprint(jax.tree_util.tree_map(lambda x:type(x), lrs))
        # exit(0)
