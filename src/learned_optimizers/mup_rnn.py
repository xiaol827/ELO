...
class MuRNNMLPLOpt(lopt_base.LearnedOptimizer):
  ...
  def __init__(
      ...
      #ablation flags
      mup_to_lstm: bool = False,
      zero_lstm_features: bool = False,
  ):
    """Initializer.
    ...
    self.mup_to_lstm = mup_to_lstm
    self.zero_lstm_features = zero_lstm_features
  ...
  def opt_fn(self,
    ...
    class _Opt(opt_base.Optimizer):
      ...
      def update_is_training(self, opt_state, grads, loss,
        ...
        ################### VALID LOSS PATCH ###################
        valid_loss = jnp.nan_to_num(valid_loss) # FIXME: This is a patch to avoid NaNs in the valid_loss
        ################### VALID LOSS PATCH ###################
        ...
        if parent.zero_lstm_features:
          ff_inputs = jax.tree_util.tree_map(
              lambda x: jnp.zeros_like(x), ff_inputs)
        ...
        for o, v, mup_lr in zip(
          ...
          if parent.mup_to_lstm:
            to_lstm = to_lstm * mup_lr
