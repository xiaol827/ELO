...
def scale(image, factor, replace):
  """Isotropic scale around image center by `factor`.

  factor < 1.0 zooms out (image shrinks, borders filled with `replace`);
  factor > 1.0 zooms in (image enlarges, edges cropped).
  """
  ...
  image = contrib_image.transform(
      wrap(image), [inv_s, 0., a2, 0., inv_s, b2, 0., 0.])
  return unwrap(image, replace)
...
def _scale_level_to_arg(level):
  """Linear in user magnitude: m=10 → ±0.1, m=30 → ±0.8 (clipped)."""
  scale_limit = max(0.0, min(0.95, (level - 10.0) * 0.035 + 0.1))
  signed = _randomly_negate_tensor(scale_limit)
  return (1.0 + signed,)  # factor ∈ {1 - scale_limit, 1 + scale_limit}
