...
def create_learning_rate_schedule(
  ...
  def step_fn(step):
    ...
    if decay_type in ("linear", "polynomial"):
      ...
      # Optional floor: `end` (absolute lr) or `min_lr_factor` (fraction of base lr).
      end = kw.get("end", kw.get("min_lr_factor", 0.0) * lr)
      lr = end + (lr - end) * 0.5 * (1. + jnp.cos(jnp.pi * progress))
...
class BigVisionMetricWriter:
  """A class for logging metrics with optional wandb support."""

  def __init__(self, xid=-1, wid=-1, workdir=None, config=None, resume=False):
    self.step_start(0)
    self.wandb_run = None
    self._config = config
    ...
    if config and config.get("wandb", {}).get("enabled", False):
      self._init_wandb(config, workdir, resume=resume)

  def _init_wandb(self, config, workdir, resume=False):
    """Initialize Weights & Biases logging.

    `resume` is the source of truth (driven by --resume CLI flag). When True,
    we continue the previous WandB run by reading workdir/wandb_run_id.txt.
    When False, we start a fresh run and overwrite any stale id file so future
    --resume picks up the new id. The legacy `resume_wandb` config field is
    ignored.
    """
    ...
    if group is None:
      group = _generate_wandb_group_name(config)

    wandb_kwargs = dict(
        project=wandb_config.get("project", "big_vision"),
    ...
    if resume:
      run_id = self._get_or_create_wandb_id(
          workdir, explicit_id=os.environ.get("WANDB_RUN_ID"))
      wandb_kwargs["id"] = run_id
    elif workdir:
      # Force a fresh run; clear any stale id from a previous workdir use.
      stale = os.path.join(workdir, "wandb_run_id.txt")
      if gfile.exists(stale):
        gfile.remove(stale)

    self.wandb_run = wandb.init(**wandb_kwargs)

    # Persist the new run id so a future --resume can pick it up.
    if not resume and workdir and self.wandb_run is not None:
      gfile.makedirs(workdir)
      with gfile.GFile(os.path.join(workdir, "wandb_run_id.txt"), "w") as f:
        f.write(self.wandb_run.id)

    logging.info(f"Initialized wandb run: {self.wandb_run.name} "
                 f"(id: {self.wandb_run.id}, group: {group}, resume={resume})")

  def _get_or_create_wandb_id(self, workdir, explicit_id=None):
    if explicit_id:
      return explicit_id
    ...
    wandb_id_file = os.path.join(workdir, "wandb_run_id.txt")
    if gfile.exists(wandb_id_file):
      with gfile.GFile(wandb_id_file, "r") as f:
        return f.read().strip()

    run_id = uuid.uuid4().hex
    gfile.makedirs(workdir)
    with gfile.GFile(wandb_id_file, "w") as f:
      f.write(run_id)
    return run_id
  ...
  def step_end(self):
    ...
    if self.wandb_run is not None:
      self._log_to_wandb(self.step_metrics)

  def _log_to_wandb(self, metrics):
    # Filter out non-scalar metrics (wandb doesn't handle shapes)
    wandb_metrics = {}
    for name, value in metrics.items():
      if isinstance(value, (int, float)):
        # Replace '/' with '_' for wandb compatibility but keep structure
        # Also handle special prefixes for better organization
        wandb_metrics[name] = value

    if wandb_metrics:
      self.wandb_run.log(wandb_metrics, step=self.step)
...
def _config_to_dict(config):
  """Convert an ml_collections ConfigDict to a regular nested dict for wandb."""
...
def _generate_wandb_group_name(config):
  """Generate a wandb group name from config parameters."""
  parts = []
  
  # Optimizer name
  if config.get("optax_name"):
    parts.append(f"oname_{config.optax_name}")
  
  # Learning rate
  if config.get("lr") is not None:
    parts.append(f"lr_{config.lr}")
  
  # Batch size (nested in input config)
  if config.get("input", {}).get("batch_size") is not None:
    parts.append(f"bs_{config.input.batch_size}")
  
  # Weight decay
  if config.get("wd") is not None:
    parts.append(f"wd_{config.wd}")
  
  # Total epochs
  if config.get("total_epochs") is not None:
    parts.append(f"epochs_{config.total_epochs}")
