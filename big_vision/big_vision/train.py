...
################################################################################
  ...
  if first_step in (total_steps, 0):
    ...
    mw.step_end()  # Flush metrics to file and wandb
...
################################################################################
  ...
  for step, batch in zip(range(first_step + 1, total_steps + 1), train_iter):
    ...
    if (u.itstime(step, get_steps("log_training"), total_steps, host=0)
      ...
      # Log the actual per-step learning rate (config.lr * schedule scale).
      # Pull the JAX scalar to host before multiplying to avoid the implicit
      # host->device transfer that JAX's transfer guard would otherwise reject.
      for i, sched_fn_cpu in enumerate(sched_fns_cpu):
        mw.measure(f"lr{i if i else ''}",
                   config.lr * float(sched_fn_cpu(u.put_cpu(step - 1))))
