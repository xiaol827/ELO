...
flags.DEFINE_boolean("resume", default=False,
                     help="Resume training from existing checkpoint in workdir "
                          "and continue the previous WandB run. Default False: "
                          "start from scratch (silently overwrites any existing "
                          "checkpoint) and start a NEW WandB run.")
...
def main(argv):
  ...
  # Make sure TF does not touch GPUs (must happen before any tf.data op).
  import tensorflow as tf
...
################################################################################
  ...
  # Fall back to config.workdir if --workdir flag is not supplied.
  workdir = flags.FLAGS.workdir or config.get("workdir", None)
  logging.info(
  ...
  if workdir:  # Always create if requested, even if we may not write into it.
    ...
    ckpt_name = config.get("wandb", {}).get("name", "default")
    save_ckpt_path = os.path.join(workdir, f"{ckpt_name}_checkpoint.bv")
...
################################################################################
  ...
  lopt, meta_params = bv_optax.build_lopt(config)
  # Lift theta out of the closure so update_fn receives it as an explicit
  # sharded input rather than inlining it as an MLIR constant (which would
  # force a d2h to materialize). opt_fn(theta) is invoked inside each trace.
  theta = u.reshard(meta_params, jax.tree.map(lambda _: repl_sharding, meta_params))
  lopt_state_shape = jax.eval_shape(
      lambda t, p: lopt.opt_fn(t).init(p, num_steps=total_steps),
      theta, params_shape)
...
################################################################################
  ...
  train_state_shape = {"params": params_shape, "lopt_state": lopt_state_shape}
  ...
  lopt_state = jax.jit(
      lambda t, p: lopt.opt_fn(t).init(p, num_steps=total_steps),
      out_shardings=train_state_sharding["lopt_state"],
  )(theta, params)
  ...
  train_state = nn.unbox({"params": params, "lopt_state": lopt_state})
  del params, lopt_state  # Delete to avoid memory leak or accidental reuse.
...
################################################################################
  ...
  def update_fn(train_state, theta, rng, batch):
    """Update step."""

    images, labels = batch["image"], batch["labels"]

    step_count = train_state['lopt_state'].iteration
    ...
    params, lopt_state = train_state["params"], train_state["lopt_state"]
    # Bind theta inside the trace so the HLO sees theta as a sharded input
    # rather than a baked-in constant — no implicit transfers.
    bound_opt = lopt.opt_fn(theta)
    loss, grads = jax.value_and_grad(loss_fn)(params)
    new_opt_state = bound_opt.update(opt_state=lopt_state, grad=grads, loss=loss)
    new_params = bound_opt.get_params(new_opt_state)

    measurements = {"training_loss": loss}
    # The LO maintains its own absolute scheduled LR (warmup → cosine/constant).
    # Log it directly so wandb shows the real per-step LR, not bv_optax's unused schedule.
    measurements["lr"] = new_opt_state.scheduled_lr
    ...
    updates = jax.tree.map(lambda p_new, p_old: p_new - p_old, new_params, params)
    us = jax.tree.leaves(updates)
    measurements["l2_updates"] = jnp.sqrt(sum([jnp.sum(u * u) for u in us]))

    return {"params": new_params, "lopt_state": new_opt_state}, measurements
...
################################################################################
  ...
  if flags.FLAGS.resume:
    ...
    write_note(f"Found existing checkpoint at {save_ckpt_path}-LAST but "
               f"--resume not set; will OVERWRITE on next save.")
...
################################################################################
  ...
  first_step_device = train_state["lopt_state"].iteration
  ...
  for step, batch in zip(range(first_step + 1, total_steps + 1), train_iter):
    ...
    with jax.profiler.StepTraceAnnotation("train_step", step_num=step):
      with u.chrono.log_timing("z/secs/update0", noop=step > first_step + 1):
        with mesh, nn.logical_axis_rules(sharding_rules):
          train_state, measurements = update_fn(train_state, theta, rng_loop, batch)
    ...
    if (u.itstime(step, get_steps("log_training"), total_steps, host=0)
      ...
      # The bv_optax schedule is built but unused for LO updates — the LO's own
      # `scheduled_lr` (logged from update_fn) is the true per-step LR.
