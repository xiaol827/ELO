...
class TruncatedPES(gradient_learner.GradientEstimator):
  ...
  def compute_gradient_estimate(  # pytype: disable=signature-mismatch  # overriding-parameter-type-checks
    ...
    # import pprint
    # print('p_state')
    # pprint.pprint(jax.tree_util.tree_map(lambda x:x.shape,state.__dict__))
    # print('vec_p_theta')
    # pprint.pprint(jax.tree_util.tree_map(lambda x:x.shape,vec_p_theta))
    # print('theta')
    # pprint.pprint(jax.tree_util.tree_map(lambda x:x.shape,theta))
    ...
    for i in range(self.trunc_length // self.steps_per_jit):
      ...
      # print(i, 'datas in PES',jax.tree_util.tree_map(lambda x:x.shape, datas))
    ...
    if with_summary:
      ...
      # max_ = functools.reduce(lambda x, y: jax.tree_util.tree_map(max, x, y), max_list)
      # min_ = functools.reduce(lambda x, y: jax.tree_util.tree_map(max, x, y), min_list)
